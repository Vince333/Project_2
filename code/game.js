...
var actorChars = {
  "@": Player,
  "o": Gem, // A Gem will wobble up and down
  "=": Spider,
  "|": Arrow,
  "v": Boulder,
  "k": Key
...
Boulder.prototype.type = "boulder";

function Spider(pos, ch) {
  this.pos = pos;
  this.size = new Vector(1.5, 1);
  if (ch == "=") {
    // Horizontal spider moving
...
Boulder.prototype.act = function(step, level) {
...
Spider.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if ((type == "spider" || type == "boulder" || type == "arrow" || type == "lava") && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "gem" || type == "key") {
    this.actors = this.actors.filter(function(other) {
      return other != actor;
    });
    // If there aren't any gems left, player wins
    if (!this.actors.some(function(actor) {
           return actor.type == "gem" || actor.type == "key";
         })) {
      this.status = "won";
      this.finishDelay = 1;
    }
